...
script
    cd #{current_path} && exec sudo su - #{node_user} -c 'NODE_ENV=#{node_env} #{forever_binary} start #{current_path}/#{app_command} 2>> #{stderr_log_path} 1>> #{stdout_log_path}'
end script

pre-stop script
    cd #{current_path} && exec sudo su - #{node_user} -c 'NODE_ENV=#{node_env} #{forever_binary} stop #{current_path}/#{app_command}'
end script
EOD
  }
